...
- 読み取り: FETCH_DATA で on-demand hydration
- 書き戻し: NOTIFY_FILE_CLOSE_COMPLETION で modified 検出 → ロック取得 → アップロード →
  `UpdatePlaceholder` でメタデータ同期 → `SetInSyncState(true)` → ロック解放 → dehydrate
- ロック: 書き込み発生時のみ取得。30 秒間隔の heartbeat で延長
